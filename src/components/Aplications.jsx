...
export default function Applications() {
  ...
  const [bigCounter, setBigCounter] = useState(0); // 🔥 licznik dla "21"
  ...
  useEffect(() => {
    const startCounters = () => {
      ...
      // 🔥 "21" wolniej, np. 3000ms
      animateValue(0, 21, 3000, setBigCounter);

      // 🔥 małe liczniki szybciej, np. 1500ms
      stats.forEach((stat, idx) => {
        animateValue(0, stat.number, 1500, (val) => {
          setCounters((prev) => {
  ...
  return (
    <div
      className="w-full flex flex-col items-center py-20 bg-white"
      ref={sectionRef}
    >
      <div className="w-full max-w-7xl flex flex-col lg:flex-row items-start gap-12 px-6 md:px-0">
        {/* 🔥 Liczba 21 z wolniejszą animacją */}
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-6 lg:gap-8 w-full">
          ...
          <div className="flex-1 max-w-full lg:max-w-[400px] flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <button className="relative flex h-[50px] w-40 items-center justify-center overflow-hidden bg-green-600 font-medium text-white shadow-2xl transition-all duration-300 before:absolute before:inset-0 before:border-0 before:border-white before:duration-100 before:ease-linear hover:bg-white hover:text-green-600 hover:shadow-green-600 hover:before:border-[25px]">
          ...
          <div className="w-full lg:w-3/5 grid lg:grid-cols-2 grid-cols-1 text-center gap-3">
            {stats.map((item, idx) => (
              <div
                key={idx}
                className={`group p-3 lg:p-5 lg:first:border-t-0 lg:border-gray-300 ${
                  idx === 0 || idx === 1 ? "lg:border-b border-gray-300" : ""
                } ${idx === 0 || idx === 2 ? "lg:border-r border-gray-300" : ""} flex flex-col items-center justify-center cursor-pointer`}
              >
